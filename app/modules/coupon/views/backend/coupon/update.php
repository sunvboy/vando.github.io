<section class="content-header">
	...
	<ol class="breadcrumb">
		...
		<li class="active"><a href="<?php echo site_url('coupon/backend/coupon/update/'.$Detailcoupon['id']);?>">Chỉnh sửa bản ghi</a></li>
	</ol>
</section>
							...
							<div class="box-body">
								<div class="box-form-center">
									<div class="form-group">
										...
										<div class="col-sm-8">
											<?php echo form_input('couponCode', set_value('couponCode', $Detailcoupon['couponCode']), 'class="form-control" id="couponCode" placeholder="Ví dụ: COUPON10%"');?>
										</div>
									...
									<div class="form-group">
										<div class="col-sm-6">
											<div class="form-group">
												...
												<div class="col-sm-12">
													<?php echo form_dropdown('couponType', $this->configbie->data('couponType'), $Detailcoupon['couponType'], 'class="form-control select2" style="width: 100%;" id="couponType"');?>
												</div>
											</div>
										</div>
										<div class="col-sm-6">
											<div class="form-group">
												...
												<div class="col-sm-12">
													<div class="input-group">
														<?php echo form_input('couponTypeValue', $Detailcoupon['couponTypeValue'], 'class="form-control couponTypeValue"');?>
								...
								<div class="box-form-center">
									<div class="form-group">
										<div class="col-sm-12">
	                                        <input class="next-checkbox checkbox-item" type="checkbox" <?php echo (($checkPurchase == 1) ? 'checked="checked"' : '') ?> value="1" name="requiresMinimumPurchase" id="discount_next_requires_minimum_purchase">
	                                    ...
	                                    <div class="col-sm-12 hide mb15" id="minimumPurchase">
	                                    	<div class="input-group">
												<?php echo form_input('minimumPurchase', set_value('minimumPurchase', $Detailcoupon['minimumPurchase']), 'class="form-control" placeholder=""');?>
	                                            ...
	                                            <ul>
	                                                <li>
	                                                    <div class="next-input-wrapper">
	                                                        <input <?php echo (($checkAppliesTo == 'cart') ? 'checked="checked"' : '') ?> value="cart" type="radio" class="next-radio checkbox-item" name="appliesTo" id="discount_next_applies_to_cart">
	                                                        <label class="next-label-radio next-label--switch" for="discount_next_applies_to_cart">Tất cả đơn hàng</label>
	                                                    </div>
	                                                </li>
	                                                <li>
	                                                    <div class="next-input-wrapper">
	                                                        <input <?php echo (($checkAppliesTo == 'collections') ? 'checked="checked"' : '') ?> value="collections" type="radio" class="next-radio checkbox-item" name="appliesTo" id="discount_next_applies_to_collections">
	                                                        <label class="next-label-radio next-label--switch" for="discount_next_applies_to_collections">Danh mục sản phẩm</label>
	                                                    </div>
	                                                </li>
	                                                <li>
	                                                    <div class="next-input-wrapper">
	                                                        <input <?php echo (($checkAppliesTo == 'products') ? 'checked="checked"' : '') ?> value="products" type="radio" class="next-radio checkbox-item" name="appliesTo" id="discount_next_applies_to_products">
										...
										<div class="col-sm-12 hide chose-type mb15" id="products_show">
											...
											<div class="products-resource">
												<ul class="next-list--divided next-list--divided--top" id="js-products-container">
													<?php  
														if (!empty($Detailcoupon['ProductsId'])) {
															$products = $this->Autoload_Model->_get_where(array(
														    	'select' => 'id, title',
														    	'table' => 'products',
														    	'where' => array('trash' => 0, 'publish' => 1),
														    	'order_by' => 'order asc, id desc',
														    	'where_in' => explode('-', $Detailcoupon['ProductsId']),
														    	'where_in_field' => 'id',
														    ), TRUE);
														    if (isset($products) && is_array($products) && count($products)) {
														    	foreach ($products as $key => $val) {
														    		echo '<li data-id="'.$val['id'].'" class="items-remove active">';
																        echo '<div class="mt-flex mt-flex-middle mt-flex-space-between">';
													                        echo '<div class="ui-stack-item ui-stack-item--fill">'.$val['title'].'</div>';
													                        echo '<div class="ui-stack-item close-item"><i class="fa fa-close"></i></div>';
													                    echo '</div>';
																    echo '</li>';
														    	}
														    }
												...
												<input type="hidden" name="ProductsId" value="<?php echo $Detailcoupon['ProductsId'] ?>" id="ProductsId">
                                            </div>
										</div>
										<div class="col-sm-12 hide chose-type mb15" id="collections_show">
											...
											<div class="collection-resource">
												<ul class="next-list--divided next-list--divided--top" id="js-collections-container">
													<?php  
														if (!empty($Detailcoupon['CollectionId'])) {
															$catalogues = $this->Autoload_Model->_get_where(array(
														    	'select' => 'id, title',
														    	'table' => 'products_catalogues',
														    	'where' => array('trash' => 0, 'publish' => 1),
														    	'order_by' => 'order asc, id desc',
														    	'where_in' => explode('-', $Detailcoupon['CollectionId']),
														    	'where_in_field' => 'id',
														    ), TRUE);
												...
												<input type="hidden" name="CollectionId" value="<?php echo $Detailcoupon['CollectionId'] ?>" id="CollectionId">
                                            </div>
										</div>
									</div>
								</div>
								...
								<div class="box-form-center">
		                            <div class="form-group">
										...
										<div class="col-sm-12">
	                                        <input class="next-checkbox checkbox-item" type="checkbox" <?php echo (($checkLimitTotalUse == 1) ? 'checked="checked"' : '') ?> value="1" name="limitTotalUse" id="discount_next_limit_total_use">
	                                    ...
	                                    <div class="col-sm-12 hide mb15" id="limitedUseSelected">
											<?php echo form_input('limitedUseTotal', set_value('limitedUseTotal', $Detailcoupon['limitedUseTotal']), 'class="form-control" placeholder="Số lần sử dụng"');?>
										</div>
									</div>
								</div>
								...
								<div class="box-form-center">
		                            <div class="form-group" id="scheduleStartDate">
		                            	<?php $date_start = explode(' ', $Detailcoupon['date_start']); ?>
		                            	...
		                            	<div class="col-sm-6">
											...
											<div class="input-group">
												...
												<input type="text" name="" class="form-control datetimemonth change_time" value="<?php echo ((!empty($date_start[0])) ? convert_time_3($date_start[0]) : date('d/m/Y', time() + 7*3600))  ?>">
											</div>
										</div>
										<div class="col-sm-6">
											...
											<div class="input-group">
												...
												<input type="text" name="" class="form-control datetimeseconds change_time" value="<?php echo ((!empty($date_start[1])) ? substr($date_start[1], 0, 5) : date('h:i', time() + 7*3600)) ?>">
											</div>
										</div>
									...
									<div class="form-group" style="margin-bottom: 0">
										<div class="col-sm-12">
	                                        <input class="next-checkbox checkbox-item" type="checkbox" value="1" <?php echo (($checkScheduleEndDate == 1) ? 'checked="checked"' : '') ?> name="scheduleEndDate" id="discount_next_schedule_end_date">
	                                        <label class="next-label-checkbox next-label--switch" for="discount_next_schedule_end_date">
	                                            Thời gian kết thúc
	                                        </label>
	                                    </div>
	                                    <div id="scheduleEndDate" class="hide">
	                                    	<?php $date_end = explode(' ', $Detailcoupon['date_end']); ?>
											<div class="col-sm-6 mb15">
												...
												<div class="input-group">
													...
													<input type="text" name="" class="form-control datetimemonth change_time" value="<?php echo ((!empty($date_end[0])) ? convert_time_3($date_end[0]) : date('d/m/Y', time() + 7*3600))  ?>">
												</div>
											</div>
											<div class="col-sm-6 mb15">
			                                    <div class="mb10">Thời điểm</div>
												<div class="input-group">
													...
													<input type="text" name="" class="form-control datetimeseconds change_time" value="<?php echo ((!empty($date_end[1])) ? substr($date_end[1], 0, 5) : date('h:i', time() + 7*3600))  ?>">
												</div>
											</div>
						...
						<div class="tab-pane active" id="tab-advanced">
							...
							<div class="box-body" id="result_value">
								...
								<ul class="" id="list-coupon-item">
									...
									<li class="ui-content-list__item hide">Áp dụng từ <span id="time_apply">5:05</span> <span id="to_day"></span> <span id="time_apply_end"></span></li>
								</ul>
						    </div>
						</div>
						<div class="clearfix"></div>
...
<script type="text/javascript">
	$(function(){
		...
		function load_checkbox(){
			...
			load_miniumpurchase();
			load_sale_type();
			load_limitedUseTotal();
		}
		load_apply_to_check();
		$('.next-radio').click(function(){
			load_apply_to_check();
		});
		function load_apply_to_check(){
			var flag = $('input[name="appliesTo"]:checked').val();
		...
		function load_type_value(){
			var result = $('#couponType').val();
			if (result == 0) {
		...
		// Check Apply to cart 
		function load_miniumpurchase(){
			var value = $('input[name="minimumPurchase"]').val();
			...
			$('input[name="requiresMinimumPurchase"]').each(function(){
				if($(this).is(':checked')) {
					$('#total_order').parent().removeClass('hide');
				}else {
					$('#total_order').parent().addClass('hide');
		...
		function load_limitedUseTotal(){
			var value = $('input[name="limitedUseTotal"]').val();
			$('#total_apply').html(number_format(value, 0, '.', '.'));
			if (value == '') {
            	$('#total_apply').parent().addClass('hide');
			}else{
			...
			$('input[name="limitTotalUse"]').each(function(){
				if($(this).is(':checked')) {
					$('#total_apply').parent().removeClass('hide');
				}else {
					$('#total_apply').parent().addClass('hide');
