<section class="content-header">
	...
	<ol class="breadcrumb">
		...
		<li class="active"><a href="<?php echo site_url('projects/backend/projects/view');?>">dự án</a></li>
	</ol>
</section>
  ...
  <div class="row">
	<div class="col-xs-12">
	  <div class="box">
		<div class="box-header">
			<h3 class="box-title pull-right">
				<div class="btn-group add-sort">
					...
					<a href="<?php echo site_url('projects/backend/projects/create');?>" class="btn btn-add  btn-flat"><i class="fa fa-plus"></i> Thêm dự án</a>
				</div>
			</h3>
			<div class="box-tools pull-left">
				<span class="gcheckAction pull-left btn btn-default" data-module="projects"><i class="fa fa-trash"></i></span>
				<form class="pull-left" method="get" action="<?php echo site_url('projects/backend/projects/view');?>">
		...
		<?php if(isset($Listprojects) && is_array($Listprojects) && count($Listprojects)){ ?>
		<div class="box-body table-responsive no-padding">
			<form method="post" action="" id="fcFrom">
			<table class="table" id="diagnosis-list">
				<tr>
					<th class="width-checkbox uk-text-center" style="width: 50px;">
				...
				<?php 
					...
					$href = rewrite_url($item['canonical'], $item['slug'], $item['id'], 'projects');
					
				?>
				<tr>
					<td class="uk-text-left" style="width: 50px;text-align: left;">
							...
							<div class="col-sm-9">
								...
								<div class="meta">
									<span class="user-create">
										...
										<a href="<?php echo site_url('customers/backend/customers/update/'.$item['userid_created'].'') ?>" title=""><?php echo $item['fullname']; ?></a>
									</span>
								...
								<span class="user-create"><i class="fa fa-user"></i> <a href="<?php echo site_url('projects/backend/projects/view?userid='.$item['userid_updated'].'') ?>" title=""><?php echo $item['fullname_update']; ?></a></span>
								 <?php } ?>
								<?php foreach($_catalogue_list as $keyCat => $valCat){ ?>
								<span><i class="fa fa-folder-open"></i> <a href="<?php echo site_url('projects/backend/projects/view?cataloguesid='.$valCat['id'].''); ?>" title=""><?php echo $valCat['title']; ?></a></span>
								<?php }} ?>
								</div>
							</div>
						</article>
					...
					<?php $list = $this->configbie->data('measure'); ?>
						<input type="text" style="text-align:center;width:50px;margin-right:10px;margin-bottom:10px;" value="<?php echo $item['price']; ?>" class="price" data-module="price" data-id="<?php echo $item['id'] ?>"  /><?php echo $list[$item['measure']]  ?>
					</td>
					<td><?php echo $item['viewed']; ?></td>
					<td style="text-align:right;"><?php echo form_input('order['.$item['id'].']', $item['order'], 'data-module="projects" data-id="'.$item['id'].'"  class="form-control sort-order" placeholder="Vị trí" style="width:50px;text-align:center;display:inline;"');?></td>
					<td>
						<a href="<?php echo site_url('projects/backend/projects/set/publish/'.$item['id'].'?redirect='.urlencode(current_url())); ?>" title="" class="status-publish">
							<img src="<?php echo ($item['publish'] > 0)? 'templates/backend/images/publish-check.png':'templates/backend/images/publish-deny.png'; ?>" alt="" />
						</a>
					</td>
					<td>
						<a href="<?php echo site_url('projects/backend/projects/set/isaside/'.$item['id'].'?redirect='.urlencode(current_url())); ?>" title="" class="status-publish">
					...
					<td class="text-center">
						<div class="btn-group" style="min-width: auto">
							<a href="<?php echo site_url('projects/backend/projects/delete/'.$item['id']).'?redirect='.urlencode(current_url());?>" class="btn btn-danger"><span class="fa fa-trash"></span></a>
							<a href="<?php echo site_url('projects/backend/projects/update/'.$item['id']).'?redirect='.urlencode(current_url());?>" class="btn btn-success"><i class="fa fa-edit"></i></a>
						</div>
					 </td>
				</tr>
				<?php } ?>
...
$(document).ready(function(){
	$('#btnsort').click(function(){
		$.post('<?php echo site_url('projects/ajax/projects/sort')?>', $('#fcFrom').serialize(), function(data){
			location.reload();
		})
		return false;
	})
...
<script type="text/javascript"> 
	...
	$(document).ready(function(){
		...
		$('.price').on('keyup', function(event) {
			...
			var url = 'projects/ajax/projects/fast_price_update';
			clearTimeout(time);
