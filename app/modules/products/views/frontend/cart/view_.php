							...
							<div class="col-md-7 col-sm-12 col-xs-12">
								<div class="uk-panel customer mb20">
									...
									<div class="panel-body">
										...
										<?php  
											$shipcode = array(
												'shop' => 'Giao hàng tận nơi(COD)',
												'inner' => 'Giao hàng tận nơi(Chuyển khoản trước)',
											);
										?>
									...
									<div class="panel-body" style="padding: 0">
													...
													<li class="item uk-clearfix">
														...
														<div class="colimg uk-float-left">
															<a href="javascript: void(0)" class="img img-scaledown" target="_blank">
																<img src="<?php echo getthumb($val['detail']['images']);?>" alt="<?php echo $val['name']; ?>" />
															</a>
														</div>
														<div class="colinfo uk-float-right">
															<div class=" uk-flex uk-flex-space-between">
																<div class="title ec-line-3">
																	<a href="javascript: void(0)" class="link" target="_blank"><?php echo $val['name']; ?> </a>
										...
										<div class="total">
											...
											<div class="tt-price uk-flex uk-flex-middle uk-flex-space-between" id="total_total">
												<input type="hidden" name="total_cart_money" value="<?php echo ($this->cart->total() - $totalsale - $totalKGVPT) ?>">
												<div class="title"><?php echo $this->lang->line('payment_money_after') ?></div>
												<div class="price_tt"><strong style="font-size: 17px;"><?php echo str_replace(',', '.', number_format($this->cart->total() - $totalsale - $totalKGVPT)) ?> đ</strong></div>
											</div>
										</div>
									</div>
								</div><!-- .uk-panel -->
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		function load_total_price_cart(){
			...
			$('#total_total .price_tt strong').html('').html(number_format((shipcode + total - giftcode - salesoluong - saleKGVPT), 0, '.', '.')+' ₫');
			$('#total_total').attr('data-price', (shipcode + total - giftcode - salesoluong - saleKGVPT));
			$('#total_total').find('input').attr('value', (shipcode + total - giftcode - salesoluong - saleKGVPT));
			load_coint_cart();
		}
