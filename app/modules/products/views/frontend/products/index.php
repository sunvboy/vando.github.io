        ...
        <div class="container">
            <div class="row">
                <div class="col-md-5 mb25">
                    <div class="slick-slider12 player_ <?php echo((!empty($DetailProducts['source'])) ? 'active_stop' : '') ?>" data-source="<?php echo $DetailProducts['source'] ?>">
                    ...
                    <div class="products-hidden-">
                        <div class="mt-flex mt-flex-middle mt-flex-space-between">
                            <div class="item-">
                                <?php if (!empty($DetailProducts['source'])): ?>
                                    <span class="player_button click_tog active" data-show="2"><i class="fab fa-google-play"></i> Videos</span>
                                <?php endif ?>
                                <span  class="gallerys_button click_tog <?php echo((!empty($DetailProducts['source'])) ? '' : 'active') ?>" data-show="1"><i class="far fa-image"></i> Hình ảnh</span>
                            </div>
                            <div class="item-">
                            ...
                            <?php if (isset($customer) && is_array($customer) && count($customer)): ?>
                                <div class="getlink_affiliate mt-flex mt-flex-middle mt15 mb5">
                                    ...
                                    <span class="relative aff_right">
                                        <input type="text" readonly="" value="<?php echo $canonical . '?aff=' . $customer['affiliate_id'] ?>"  class="form-control" id="affiliate_link" style="padding-right: 75px;">
                                        <button type="button" class="btn btn-success" onclick="getlink_affiliate();">Lấy link
                                        </button>
                                    </span>
                                </div>
                                <script>
                                ...
                                <script>
                                    $(document).ready(function(){
                                        ...
                                        function load_attr_advanced(){
                                            ...
                                            // Load albums ảnh nếu có
                                            $.post('<?php echo site_url("products/ajax/products/load_album_products");?>', {text: text.slice(0, -1), id: '<?php echo $DetailProducts['id'] ?>'}, function (data) {
                                                var json = JSON.parse(data);
                                                $('.ajax-addtocart.single_product').attr('data-id',json.productsid);
                                                if (json.error == true) {
                                                    $('.slick-slider').html(json.html);
                                                    $('.count-img').html(json.count);
                                                }
//                                                load_id_products_add_to_cartall();
                                            });

                                            // Load số lượng + giá vv
                                            $.post('<?php echo site_url("products/ajax/products/load_price_versions_products");?>', {text: text.slice(0, -1), id: '<?php echo $DetailProducts['id'] ?>'}, function (data) {
                                                var json = JSON.parse(data);
                                                if (json.error == false) {
                                                ...
                                                load_products_count_priceall();
                                            });

                                            $('.ajax-addtocart.single_product, .ajax-addtocart-all').attr('data-option', outputText.slice(0, -1));
                                        }

                                        function load_products_count_priceall() {
                                            var outputText3 = 0;
                                            var outputText13 = 0;
                                            $('.images-text-link li a').each(function () {
                                                var divHtml3 = parseInt($(this).attr('data-price'));
                                                outputText3 = (outputText3 + divHtml3);

                                                var divHtml13 = parseInt($(this).attr('data-price-old'));
                                                outputText13 = (outputText13 + divHtml13);
                                            });
                                            $('.text-button-images .saleoff').html(number_format(outputText3, 0, '.', '.') + 'đ');
                                            $('.text-button-images .priceold').html(number_format(outputText13, 0, '.', '.') + 'đ');
                                        }

                                        function load_id_products_add_to_cartall() {
                                            var outputText2 = '';
                                            $('.images-text-link .ajax-addtocart').each(function () {
                                                var divHtml2 = $(this).attr('data-id');
                                                outputText2 += divHtml2 + '-';
                                            });
                                            $('.ajax-addtocart-all').attr('data-id', outputText2.slice(0, -1));
                                        }
                                    })
                                </script>
                            <?php endif ?>
...
<div id="dt-fixed-buynow">
    ...
    <a class="checkout ajax-addtocart single_product" data-id="<?php echo $DetailProducts['id'] ?>" data-option="" data-quantity="1">MUA NGAY</a>
</div>
