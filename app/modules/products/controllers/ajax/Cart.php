...
//	}
	public function ajax_location(){
		...
		if($quanhuyen==0){
			...
			$result = location_dropdown('Phường/Xã', array('parentid' => $cityid));
	...
	public function addtocartv(){
		...
		$data = array(
			...
			'qty' => $quantity,
			'price' => $price,
			'options' => array('Size' => $size, 'Màu sắc' => $color, 'sanphamtangkem'=> $sanpham)
		);
		...
		foreach($cart as $key => $val){
			...
			$price = $val['price'];
			$html = $html . '<tr class="item_2" data-id="1041431100">
				<td class="img">
					<a href="'.$val['detail']['href'].'"><img src="'.getthumb($val['detail']['images']).'" alt="'.$val['name'].'"></a>
				</td>
				<td>
					<input name="quantity" value="'.$val['rowid'].'" class="quantity ajax-quantity" type="hidden">
					<a class="pro-title-view" href="'.$val['detail']['href'].'" >'.$val['name'].'</a>
					<span class="pro-price-view">'.str_replace(',', '.', number_format($price)).'₫</span>
					<span class="pro-price-view">Số lượng: '.$val['qty'].'</span>
					<div class="variant">';
			if(!empty($val['options']['sanphamtangkem'])) {
				...
				$html = $html . '<span class="pro-price-view">Sản phẩm tặng kèm: '.$sanphamtangkem['title'].'</span>';
			}
	...
	public function addalltocart(){
		...
		$price = $this->input->post('price');
		$this->load->model('combos/BackendCombos_Model');
		$products = $this->BackendCombos_Model->readfrontend($id);
