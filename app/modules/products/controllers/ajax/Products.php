...
class Products extends FC_Controller{
					...
					$html .= '<table class="table-reponsive table">';
                        $html .= '<thead>';
                            $html .= '<tr>';
                                ...
                                $html .= '<th><span class="options-header">SKU</span></th>';
                                ...
                                $html .= '<th style="min-width:100px"><span></span></th>';
                            $html .= '</tr>';
                        $html .= '</thead>';
                        $html .= '<tbody>';
							foreach ($arr_arr as $val) {
								...
								$html .= '<tr>';
	                                ...
	                                $html .= '<td>'.$val.'</td>';
	                                $html .= '<td style="width:100px"><input class="form-control" type="text" name="version[sku][]"></td>';
	...
	public function load_album_products(){
		...
		$count = 0;
		$text = $this->input->post('text');
		$id = $this->input->post('id');$alert['productsid'] = $id;
		$html = '';
		if (!empty($text)) {
			$result = $this->Autoload_Model->_read(array(
				'select' => 'id, title, images, albums, source, parentid',
				'table' => 'products',
				'where' => array('publish' => 1, 'trash' => 0, 'title' => $text, 'parentid' => $id),
			...
			if (isset($result) && is_array($result) && count($result)) {
				...
				$alert['html'] = $html;
				$alert['count'] = $count;
				$alert['productsid'] = $result['id'];
				$alert['error'] = true;
			}
		}
		echo json_encode($alert); die();
	}
	public function load_album_color(){
		$alert = array(
			'error' => false,
			'html' => '',
		);
		...
		if (!empty($color)) {
			if (isset($color) && is_array($color) && count($color)){
				foreach ($color as $key=>$val){
					...
					$html .= '<div class="slick-slider12 " ><img src="'.$val['images_0'].'" alt="'.$val['title'].'"></div>';
					$html .= '<div class="slick-slider"><div id="slider-owl" class="mt-list mt-clearfix owl-carousel">';

					for ($i=0;$i<=9;$i++){
						if(!empty($val['images_'.$i.''])){
							$html .= '<div class="">';
							$html .= '<a href="javascript:void(0);" data-src="'.$val['images_'.$i.''].'">';
							$html .= '<img src="'.getthumb($val['images_'.$i.''], TRUE).'" alt="'.$val['title'].'">';
							$html .= '</a>';
							$html .= '</div>';
						}
					}
				...
				$result1 = $this->Autoload_Model->_read(array(
					'select' => 'id, title, images, albums, source',
					'table' => 'products',
					'where' => array('publish' => 1, 'trash' => 0, 'id' => $color['productsid']),
					'order_by' => 'id desc, order asc'
				));
				if (isset($result1) && is_array($result1) && count($result1)) {
					$gallerys1 = json_decode($result1['albums'], TRUE);
					if (isset($gallerys1) && is_array($gallerys1) && count($gallerys1)){
						...
						$html .= '<div>';
						$html .= '<a href="javascript:void(0);" data-src="'.$result1['images'].'">';
						$html .= '<img class="active-img" src="'.getthumb($result1['images'], TRUE).'" alt="'.$result1['title'].'" data-stt="1/'.(count($gallerys1) + 1).'">';
						$html .= '</a>';
						$html .= '</div>';
						foreach ($gallerys1 as $key => $val){
							$html .= '<div class="'.((!empty($result1['source'])) ? 'player_' : '').'">';
							$html .= '<a href="javascript:void(0);" data-src="'.$val['images'].'">';
							$html .= '<img src="'.getthumb($val['images'], TRUE).'" alt="'.$result1['title'].'" data-stt="'.($key + 2).'/'.(count($gallerys1) + 1).'">';
							$html .= '</a>';
							$html .= '</div>';
						}
						$html .= '</div>';
	...
	public function load_price_versions_products(){
		...
		$id = $this->input->post('id');
		$html = '';
		// Check phần thuộc tính của sản phẩm cha
		$total_attribute = explode(',', $text);
		$result_attr_advanced = $this->Autoload_Model->_get_where(array(
            'select' => 'id, title, attribute',
            'table' => 'products_att_advanced',
            'where' => array('productsid' => $id, 'trash' => 0),
            'order_by' => 'id asc'
        ), TRUE);

		if (!empty($id) && !empty($text) && count($total_attribute) == count($result_attr_advanced)) {
			$result = $this->Autoload_Model->_read(array(
		...
		if (isset($result) && is_array($result) && count($result)) {
			...
			$alert['price'] = $result['saleoff'];
		}
		echo json_encode($alert); die();
	}
	public function load_price_size_products(){
		$alert = array(
			'error' => TRUE,
			'result' => '',
			'price' => 0,
		...
		$result = $this->Autoload_Model->_read(array(
			'select' => 'id, price, saleoff, quantity, status, count_order',
			'table' => 'products',
			'where' => array('publish' => 1, 'trash' => 0, 'id' => $productsid),
			'order_by' => 'id desc, order asc'
		));
		$html = '';
		$price = $result['price'];
		...
		$html .= '<ul>';
		$html .= '<li>'.$gia.' '.((!empty($price) && !empty($saleoff) && $price > $saleoff) ? $giaold : '').'</li>';
		$html .= '<li><a class="'.((!empty($result['status']==1)) ? 'bg-primary' : 'bg-danger').'">'.$this->configbie->data('status', ((!empty($result['status'])) ? 0 : 1 )).'</a></li>';
	...
	public function load_version_products(){
		...
		if (!empty($parentid)) {
			$result = $this->Autoload_Model->_get_where(array(
				'select' => 'id, title, slug, canonical, images, description, price, saleoff, status, quantity, count_order',
				'table' => 'products',
				...
				$html .= '<table class="table-reponsive table">';
                    ...
                    $html .= '<tbody>';
						foreach ($result as $key => $val) {
							...
							$html .= '<tr>';
                                $html .= '<td class="select">'.$k.'<input class="form-control" type="hidden" name="version[title][]" value="'.$val['title'].'"><input class="form-control" type="hidden" name="version[id][]" value="'.$val['id'].'"></td>';
                                $html .= '<td>'.$title.'</td>';
                                $html .= '<td style="min-width:100px"><input class="form-control" type="text" name="version[price][]" value="'.$val['price'].'"></td>';
                                $html .= '<td style="min-width:100px"><input class="form-control" type="text" name="version[saleoff][]" value="'.$val['saleoff'].'"></td>';
                                $html .= '<td style="min-width:100px"><input class="form-control" type="text" name="version[quantity][]" value="'.$val['quantity'].'"></td>';
                                $html .= '<td style="min-width:100px"><input class="form-control" type="text" name="version[count_order][]" value="'.$val['count_order'].'"></td>';
                                $html .= '<td style="min-width:100px">';
                                	$html .= '<div class="btn-group" style="min-width: auto;">';
                                		$html .= '<div class="btn btn-default delete-version" data-id="'.$val['id'].'"><span class="fa fa-trash"></span></div>';
                                		$html .= '<div class="btn btn-default edit-version" data-id="'.$val['id'].'"><span class="fa fa-edit"></span></div>';
                                	$html .= '</div>';
                                $html .= '</td>';
                            $html .= '</tr>';
                            $k++;
	...
	// Search sản phẩm tặng kèm
	public function searchsanphamtangkem(){
		$keyword = $this->input->post('keyword');
		$id = $this->input->post('id');
		$html = '';
		if (!empty($keyword)) {
			$result = $this->Autoload_Model->_get_where(array(
				...
				'where' => array('publish' =>1,'parentid' => 0, 'id !=' => $id),
				'order_by' => 'id desc'
			),TRUE);
			if(isset($result) && is_array($result) && count($result)) {
				$html .= '<table class="table" id="diagnosis-list">';
	...
	public function filter(){
		...
		if($config['total_rows'] > 0){
			...
			$config['full_tag_open'] = '<div class="pagination mb30 phantrang"><ul class="pagination uk-pagination uk-pagination-right" id="ajax-pagination">';
			...
			foreach($data['listProduct'] as $key => $val){
				...
				$html = $html.'<div class="col-md-3 col-sm-6 col-xs-6">
                                    <div class="wp-item-sp">';
                                        if (!empty($price) && !empty($saleoff) && $price > $saleoff) {
                                            $html = $html.'<div class="robbin-sale">
                                                <span>sale <br> '.$number_sale.'%</span>
                                            </div>';
                                       }
										if (isset($ReadByFieldProductsColors) && is_array($ReadByFieldProductsColors) && count($ReadByFieldProductsColors)) {
											$html = $html.'<div class="wp-list-sp-slide">';
                                                 foreach ($ReadByFieldProductsColors as $keyColor => $valColor) {
                                                    $html = $html.'<div class="wp-item-sp-main ';  if ($keyColor == 0) {$html = $html.'active';} $html = $html.'">
                                                                ...
                                                                if (!empty($valColor['images_' . $i . ''])) {
                                                                    ...
                                                                    $html = $html.'<div class="item">
                                                                        <div class="wp-img-slide-sp">
                                                                            <a href="'.$href.'"
                                                                               title="'.$valColor['title'].'"><img
                                                                                    src="'.$valColor['images_' . $i . ''].'"></a>
                                                                        </div>
                                                                    </div>';
                                                                 }
                                                             }
                                            ...
                                            $list_albums = json_decode($val['albums'],TRUE);
                                                if (isset($list_albums) && is_array($list_albums) && count($list_albums)) {
                                                    $html = $html.'<div class="wp-list-sp-slide">
                                                            <div class="wp-item-sp-main active">
                                                                <div id="" class="slide-sp owl-carousel owl-theme">';
                                                                     foreach ($list_albums as $keyAB => $valAB) {
                                                                            $html = $html.'<div class="item">
                                                                                <div class="wp-img-slide-sp">
                                                                                    <a href="'.$href.'" title="'.$valAB['title'].'"><img src="'.$valAB['images'].'"></a>
                                                                                </div>
                                                                            </div>';
                                                                     }
                                                                $html = $html.'</div>
                                        ...
                                        $html = $html.'<div class="wp-add-to-cart">
                                            <a href="'. $href.'">
                                        ...
                                        <div class="wp-name-sp">
                                            <h3 class="h3-title"><a
                                                    href="'.$href.'">'.$val['title'].'</a></h3>
                                        </div>
                                        <div class="wp-price-sp">
                                            <span class="int">'.$pri_sale.'</span>
                                        </div>';
                                         if (isset($ReadByFieldProductsColors) && is_array($ReadByFieldProductsColors) && count($ReadByFieldProductsColors)) {
                                            $html = $html.'<div class="wp-list-check-color">
                                                <ul class="ul-b list-color-sp">';
                                                     foreach ($ReadByFieldProductsColors as $keyColor => $valColor) {
                                                        $html = $html.'<li class="item-color color-1 ';  if ($keyColor == 0){ $html = $html.'active'; }
														 $html = $html.'">
                                                            <span style="background: url('.$valColor['images'].')"></span>
                                                        </li>';
                                                     }
                                                $html = $html.'</ul>
                                            </div>';
...
$html = $html.'</div>
			...
			$html = $html.'<script>$(\'.slide-sp\').owlCarousel({
							loop:true,
							margin:0,
							dots:false,
							nav:true,
			...
			$html = $html.'$(".list-color-sp li").click( function() {
