...
class Backendchungchi_Model extends FC_Model{
	...
	public function ReadAll($start = 0, $limit = 0){
		$this->db->select('lichhoc_chungchi.*');
		$keyword = $this->input->get('keyword');
	...
	public function Create($user = '', $lang = 'vietnamese'){
		$data = array(
			...
			'datethi' => $this->input->post('datethi'),
			'datehet' => $this->input->post('datehet'),
		...
		$this->db->insert('lichhoc_chungchi', $data);
	...
	public function UpdateByPost($field = '', $value = 0, $user = ''){
		...
		$this->db->where(array($field => $value))->update('lichhoc_chungchi', $data);
		$result = $this->db->affected_rows();
		$this->db->flush_cache();
		return $result;
	}
	...
	public function UpdateBatchByField($data = NULL, $field = 'id'){
		$result = $this->db->update_batch('lichhoc_chungchi', $data, $field); 
		$this->db->flush_cache();
		return $result;
	}

	public function DeleteByField($field = '', $value = 0){
		$this->db->where(array($field => $value))->update('lichhoc_chungchi', array('canonical' => '', 'trash' => 1));
		$result = $this->db->affected_rows();
		$this->db->flush_cache();
		return $result;
	}
	...
	public function InsertPhotos($id = 0){
		...
		if(isset($slide['images']) && is_array($slide['images']) && count($slide['images'])){
			...
			if(isset($data) && is_array($data) && count($data)){
				$result = $this->db->insert_batch('lichhoc_chungchi_photos', $data); 
				$this->db->flush_cache();
				return $result;
			}
		}
	...
	public function ReadPhotos($id = 0){
		$data = NULL;
		$this->db->select('*');
		$this->db->from('lichhoc_chungchi_photos');
		$this->db->where(array('lichhoc_chungchiid' => $id));
		$result = $this->db->get()->result_array();
	...
	public function DeletePhotos($id = 0){
		$this->db->where(array('lichhoc_chungchiid' => $id))->delete('lichhoc_chungchi_photos');
		$result = $this->db->affected_rows();
		$this->db->flush_cache();
		return $result;
	}
