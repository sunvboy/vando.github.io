...
class BackendVideosCatalogues_Model extends FC_Model{
	...
	public function ReadAll($start = 0, $limit = 0, $lang = 'vietnamese'){
		$this->db->where(array('videos_catalogues.trash' => 0, 'videos_catalogues.alanguage' => $lang));
		$this->db->select('videos_catalogues.*, (SELECT fullname  FROM users WHERE users.id = videos_catalogues.userid_created) as user_created, (SELECT fullname  FROM users WHERE users.id = videos_catalogues.userid_updated) as user_updated');
		$keyword = $this->input->get('keyword');
		if(!empty($keyword)){
			$keyword = $this->db->escape_like_str($keyword);
			$this->db->where('(title LIKE \'%'.$keyword.'%\' OR description LIKE \'%'.$keyword.'%\')');
	...
	public function Create($user= NULL, $lang = 'vietnamese'){
		...
		$this->db->insert('videos_catalogues', $data);
	...
	public function UpdateByPost($data = NULL){
		...
		$this->db->where(array('id' => $id))->update('videos_catalogues', $data);
		$result = $this->db->affected_rows();
		$this->db->flush_cache();
		return $result;
	}
	...
	public function UpdateBatchByField($data = NULL, $field = 'id'){
		$result = $this->db->update_batch('videos_catalogues', $data, $field); 
		$this->db->flush_cache();
		return $result;
	}

	public function DeleteByField($field = '', $value = 0, $lang = 'vietnamese'){
		$this->db->where(array($field => $value))->update('videos_catalogues', array('trash' => 1, 'alanguage' => $lang));
		$result = $this->db->affected_rows();
		$this->db->flush_cache();
		return $result;
	}
