<section class="content-header">
	...
	<ol class="breadcrumb">
		...
		<li class="active"><a href="<?php echo site_url('videos/backend/videos/view');?>">Video</a></li>
	</ol>
</section>
  ...
  <div class="row">
	<div class="col-xs-12">
	  <div class="box">
		<div class="box-header">
			<h3 class="box-title pull-right">
				<div class="btn-group add-sort">
					...
					<a href="<?php echo site_url('videos/backend/videos/create');?>" class="btn btn-add  btn-flat"><i class="fa fa-plus"></i> Thêm video mới</a>
				</div>
			</h3>
			
			
			<div class="box-tools pull-left">
				<span class="gcheckAction pull-left btn btn-default" data-module="videos"><i class="fa fa-trash"></i></span>
				<form class="pull-left" method="get" action="<?php echo site_url('videos/backend/videos/view');?>">
		...
		<?php if(isset($ListVideos) && is_array($ListVideos) && count($ListVideos)){ ?>
		<div class="box-body table-responsive no-padding">
			...
			<table class="table table-hover" id="diagnosis-list">
				...
				<?php foreach($ListVideos as $key => $item){ ?>
				<?php 
					// if ($item['videos_code'] !='') {
					...
					$_catalogue_list = $this->BackendVideosCatalogues_Model->_get_where(array(
					...
					$href = rewrite_url($item['canonical'], $item['slug'], $item['id'], 'videos');
				...
				<tr>
					...
					<td style="width:650px;">
						<article class="article-view-1">
							...
							<div class="col-sm-9">
								...
								<div class="meta">
									<span class="user-create"><i class="fa fa-user"></i> <a href="<?php echo site_url('videos/backend/videos/view?userid='.$item['userid_created'].'') ?>" title=""><?php echo $item['fullname']; ?></a></span>
								...
								<span class="user-create"><i class="fa fa-user"></i> <a href="<?php echo site_url('videos/backend/videos/view?userid='.$item['userid_updated'].'') ?>" title=""><?php echo $item['fullname_update']; ?></a></span>
								 <?php } ?>
								<?php foreach($_catalogue_list as $keyCat => $valCat){ ?>
								<span><i class="fa fa-folder-open"></i> <a href="<?php echo site_url('videos/backend/videos/view?cataloguesid='.$valCat['id'].''); ?>" title=""><?php echo $valCat['title']; ?></a></span>
								<?php }} ?>
								</div>
							</div>
						</article>
					...
					<td><?php echo form_input('order['.$item['id'].']', $item['order'], 'data-module="videos" data-id="'.$item['id'].'"  class="form-control sort-order" placeholder="Vị trí" style="width:65px;"');?></td>
					<td>
						<a href="<?php echo site_url('videos/backend/videos/set/publish/'.$item['id'].'?redirect='.urlencode(current_url())); ?>" title="" class="status-publish">
							<img src="<?php echo ($item['publish'] > 0)? 'templates/backend/images/publish-check.png':'templates/backend/images/publish-deny.png'; ?>" alt="" />
						</a>
					</td>
					<td>
						<a href="<?php echo site_url('videos/backend/videos/set/highlight/'.$item['id'].'?redirect='.urlencode(current_url())); ?>" title="" class="status-publish">
					...
					<td class="text-right">
						<div class="btn-group">
							<a href="<?php echo site_url('videos/backend/videos/delete/'.$item['id']).'?redirect='.urlencode(current_url());?>" class="btn btn-default"><span class="fa fa-trash"></span></a>
							<a href="<?php echo site_url('videos/backend/videos/update/'.$item['id']).'?redirect='.urlencode(current_url());?>" class="btn btn-default"><i class="fa fa-edit"></i></a>
							<a href="<?php echo site_url('videos/backend/videos/read/'.$item['id']).'?redirect='.urlencode(current_url());?>" class="btn btn-default"><i class="fa fa-eye"></i></a>
						</div>
					 </td>
				</tr>
				<?php } ?>
...
$(document).ready(function(){
	$('#btnsort').click(function(){
		$.post('<?php echo site_url('videos/ajax/videos/sort')?>', $('#fcFrom').serialize(), function(data){
			location.reload();
		})
		return false;
	})
