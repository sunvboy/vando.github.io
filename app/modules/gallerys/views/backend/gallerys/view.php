<section class="content-header">
	...
	<ol class="breadcrumb">
		...
		<li class="active"><a href="<?php echo site_url('gallerys/backend/gallerys/view');?>">Album ảnh</a></li>
	</ol>
</section>
		...
		<div class="box-header">
			<h3 class="box-title pull-right">
				<div class="btn-group add-sort">
					...
					<a href="<?php echo site_url('gallerys/backend/gallerys/create');?>" class="btn btn-add  btn-flat"><i class="fa fa-plus"></i> Thêm album ảnh mới</a>
				</div>
			</h3>
			
			
			<div class="box-tools pull-left">
				<span class="gcheckAction pull-left btn btn-default" data-module="gallerys"><i class="fa fa-trash"></i></span>
				<form class="pull-left" method="get" action="<?php echo site_url('gallerys/backend/gallerys/view');?>">
		...
		<div class="box-body table-responsive no-padding">
			...
			<table class="table table-hover" id="diagnosis-list">
				...
				<?php 
					...
					$_catalogue_list = $this->BackendGallerys_Model->_get_where(array(
						'select' => 'id, title, slug, canonical',
						'table' => 'gallerys_catalogues',
						'where' => array('trash' => 0),
						'where_in' => $catalogue,
						'where_in_field' => 'id',
					), TRUE);
				...
				<tr>
					...
					<td style="width:650px;">
						<article class="article-view-1">
							...
							<div class="col-sm-10">
								...
								<div class="meta">
									<span class="user-create"><i class="fa fa-user"></i> <a href="<?php echo site_url('gallerys/backend/gallerys/view?userid='.$item['userid_created'].'') ?>" title=""><?php echo $item['fullname']; ?></a></span>
								...
								<span class="user-create"><i class="fa fa-user"></i> <a href="<?php echo site_url('gallerys/backend/gallerys/view?userid='.$item['userid_updated'].'') ?>" title=""><?php echo $item['fullname_update']; ?></a></span>
								 <?php } ?>
								<?php foreach($_catalogue_list as $keyCat => $valCat){ ?>
								<span><i class="fa fa-folder-open"></i> <a href="<?php echo site_url('gallerys/backend/gallerys/view?cataloguesid='.$valCat['id'].''); ?>" title=""><?php echo $valCat['title']; ?></a></span>
								<?php }} ?>
								</div>
							</div>
						</article>
					...
					<td><?php echo form_input('order['.$item['id'].']', $item['order'], 'data-module="gallerys" data-id="'.$item['id'].'"  class="form-control sort-order" placeholder="Vị trí" style="width:65px;"');?></td>
					<td>
						<a href="<?php echo site_url('gallerys/backend/gallerys/set/publish/'.$item['id'].'?redirect='.urlencode(current_url())); ?>" title="" class="status-publish">
							<img src="<?php echo ($item['publish'] > 0)? 'templates/backend/images/publish-check.png':'templates/backend/images/publish-deny.png'; ?>" alt="" />
						</a>
					</td>
					<td>
						<a href="<?php echo site_url('gallerys/backend/gallerys/set/highlight/'.$item['id'].'?redirect='.urlencode(current_url())); ?>" title="" class="status-publish">
					...
					<td class="text-right">
						<div class="btn-group">
							<a href="<?php echo site_url('gallerys/backend/gallerys/delete/'.$item['id']).'?redirect='.urlencode(current_url());?>" class="btn btn-default"><span class="fa fa-trash"></span></a>
							<a href="<?php echo site_url('gallerys/backend/gallerys/update/'.$item['id']).'?redirect='.urlencode(current_url());?>" class="btn btn-default"><i class="fa fa-edit"></i></a>
							<a href="<?php echo site_url('gallerys/backend/gallerys/read/'.$item['id']).'?redirect='.urlencode(current_url());?>" class="btn btn-default"><i class="fa fa-eye"></i></a>
						</div>
					 </td>
				</tr>
				<?php } ?>
