...
class BackendGallerysCatalogues_Model extends FC_Model{
	...
	public function CountAll($lang = 'vietnamese'){
		...
		$this->db->from('gallerys_catalogues');
	...
	public function ReadAll($start = 0, $limit = 0, $lang = 'vietnamese'){
		$this->db->where(array('gallerys_catalogues.trash' => 0, 'gallerys_catalogues.alanguage' => $lang));
		$this->db->select('gallerys_catalogues.*, (SELECT fullname  FROM users WHERE users.id = gallerys_catalogues.userid_created) as user_created, (SELECT fullname  FROM users WHERE users.id = gallerys_catalogues.userid_updated) as user_updated');
		$keyword = $this->input->get('keyword');
		if(!empty($keyword)){
			$keyword = $this->db->escape_like_str($keyword);
			$this->db->where('(title LIKE \'%'.$keyword.'%\' OR description LIKE \'%'.$keyword.'%\')');
	...
	public function Create($user= NULL, $lang = 'vietnamese'){
		...
		$this->db->insert('gallerys_catalogues', $data);
	...
	public function UpdateByPost($data = NULL){
		...
		$this->db->where(array('id' => $id))->update('gallerys_catalogues', $data);
		$result = $this->db->affected_rows();
		$this->db->flush_cache();
		return $result;
	}
	...
	public function UpdateBatchByField($data = NULL, $field = 'id'){
		$result = $this->db->update_batch('gallerys_catalogues', $data, $field); 
		$this->db->flush_cache();
		return $result;
	}

	public function DeleteByField($field = '', $value = 0){
		$this->db->where(array($field => $value))->update('gallerys_catalogues', array('trash' => 1));
		$result = $this->db->affected_rows();
		$this->db->flush_cache();
		return $result;
	}
