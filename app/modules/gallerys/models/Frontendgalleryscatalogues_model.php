...
class FrontendGallerysCatalogues_Model extends FC_Model{
	...
	public function ReadByFieldArr($field = '', $limit = 0, $lang = 'vietnamese'){
		$this->db->where(array('trash' => 0, 'alanguage' => $lang));
		$this->db->from('gallerys_catalogues');
		$this->db->where($field);
		if($limit > 0){
	...
	public function UpdateViewed($field = '', $value = 0){
		$this->db->where(array($field => $value))->set('viewed', 'viewed+1', FALSE)->update('gallerys_catalogues');
