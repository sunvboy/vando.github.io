...
class Team extends FC_Controller{
	...
	public function view($page = 1){
		$page = (int)$page;
		$config['total_rows'] = $this->BackendIdea_Model->countall($this->fclang, 1);
		if($config['total_rows'] > 0){
			$this->load->library('pagination');
			$config['base_url'] = base_url('idea/backend/team/view');
			...
			$page = $page - 1;
			$data['listSupport'] = $this->BackendIdea_Model->view(($page * $config['per_page']), $config['per_page'], $this->fclang, 1);	
		}
		$data['template'] = 'idea/backend/team/view';
		...
		if($this->input->post('create')){
			...
			if ($this->form_validation->run()){
				...
				if($flag > 0){
					...
					redirect('idea/backend/team/view');
				}
			}
		}
		$data['template'] = 'idea/backend/team/create';
		$this->load->view('dashboard/backend/layouts/home', isset($data)?$data:NULL);
	}

	public function read($id = 0){
		
		$data['Detailaddress'] = $this->BackendIdea_Model->read($id, $this->fclang, 1);
		if(!isset($data['Detailaddress']) && !is_array($data['Detailaddress']) && count($data['Detailaddress']) == 0){
			...
			redirect_custom('idea/backend/team/view');
		}
		
		$data['template'] = 'idea/backend/team/read';
		$this->load->view('dashboard/backend/layouts/home', isset($data)?$data:NULL);
	}
	...
	public function delete($id = 0){
		...
		$data['template'] = 'idea/backend/team/delete';
		$this->load->view('dashboard/backend/layouts/home', isset($data)?$data:NULL);
	}
	...
	public function set($type = NULL, $id = 0){
		...
		$id = (int)$id;
		$data['articles'] = $this->BackendIdea_Model->read($id, $this->fclang, 1);
		...
		redirect((!empty($redirect)) ? $redirect : 'idea/backend/team/view');
	}
}
