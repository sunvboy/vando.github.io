...
class Idea extends FC_Controller{
	...
	public function Index($page = 0){
		...
		$config['total_rows'] = $this->FrontendIdea_Model->Count($this->fc_lang);
		...
		if($config['total_rows'] > 0){
			...
			$page = $page - 1;
			$data['AddressList'] = $this->FrontendIdea_Model->Viewall($page * $config['per_page'], $config['per_page'], $this->fc_lang);
		}
		$data['link'] = 'documents.html';
		$data['meta_title'] = 'Documents';
		$data['canonical'] = 'documents.html';
		$data['template'] = 'idea/frontend/idea/index';
		$this->load->view('homepage/frontend/layouts/home', isset($data)?$data:NULL);
	}

	public function View($id = 0){
		$id = (int)$id;
		$DetailAddress = $this->FrontendIdea_Model->ReadByField('id', $id, $this->fc_lang);
		...
		$data['canonical'] = rewrite_url($DetailAddress['canonical'], $DetailAddress['slug'], $DetailAddress['id'], 'idea', TRUE, TRUE);
		$data['link'] = 'documents.html';
		$data['template'] = 'idea/frontend/idea/view';
		$this->load->view('homepage/frontend/layouts/home', isset($data)?$data:NULL);
