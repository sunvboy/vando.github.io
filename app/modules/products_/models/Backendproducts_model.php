...
class BackendProducts_Model extends FC_Model{
	...
	public function Create($user = '', $catalogues = NULL, $albums = '', $shipcode_data = '', $lang = 'vietnamese'){
		...
		$data = array(
			...
			'psale' => ((!empty($saleoff) && !empty($price) && $saleoff < $price) ? 1 : 0),
	...
	public function UpdateByPost($field = '', $value = 0, $user = '',  $catalogues = NULL, $albums = '', $shipcode_data = ''){
		$price = (int)str_replace('.','', $this->input->post('price'));
		$saleoff = (int)str_replace('.','', $this->input->post('saleoff'));
		$quantity = (int)$this->input->post('quantity');
		$data = array(
	...
	public function Update_TrashAttrAdvanced($id){
		$this->db->where(array('productsid' => $id))->delete('products_att_advanced');
		$result = $this->db->affected_rows();
		$this->db->flush_cache();
		return $result;
	}
	...
	public function CreateAdvancedRelation($param = ''){
		if (isset($param) && is_array($param) && count($param)) {
			foreach ($param as $key => $val) {
				if (!empty($val['title'])) {
					$data = array(
