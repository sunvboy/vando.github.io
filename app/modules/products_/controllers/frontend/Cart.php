...
class Cart extends FC_Controller
{
    ...
    {
        $cart = $this->cart->contents();
        //echo "<pre>";var_dump(json_encode($cart));die;
        $affiliate = (isset($_COOKIE['affiliate'])) ? $_COOKIE['affiliate'] : '';
        ...
        if ($this->input->post('create')) {
            ...
            if ($this->form_validation->run($this)) {
                ...
                if ($_paymentid > 0) {
                    ...
                    $productList = array();
                    if (isset($cart) && is_array($cart) && count($cart)) {
                        foreach ($cart as $key => $valc) {
                        $options = '';
                        if (isset($valc['options']) && is_array($valc['options']) && count($valc['options'])){
                            $j = 1;
                            foreach ($valc['options'] as $keyc => $vals){
                                $options .=  $keyc.':'.$vals.(($j == 1 && $j < count($valc['options'])) ? ',' : '');
                                $j++;
                            }
                        }
                            $productList[] = array(
                    ...
                    $data_arr = array(
                        ...
                        "moneyTransfer" => null,
                        "paymentMethod" => (($method == 'shop' ) ? 'Store' : 'COD'),
                        "paymentGateway" => null,
                        "paymentCode" => null,
                        "carrierId" => null, // carrierId get from get/shippingFee.php
                        "carrierServiceId" => null, // carrierServiceId get from get/shippingFee.php
                        "codFee" => 0,
                        // "shipFeeBy" => "Sender", // Receiver
                        "shipFee" => 0,
                        "customerShipFee" => $this->input->post('shipcode_value'),
                        'moneyDiscount' => $this->input->post('giftcode_value'),
                    //     "deliveryDate" => date('Y-m-d'),
                        ...
                        "height" => null,
                        "length" => null,
                        "createdDateTime" => gmdate('Y-m-d H:i:s', time() + 7 * 3600),
                        "customerName" => $this->input->post('fullname'),
                        "customerMobile" => $this->input->post('phone'),
                        "customerEmail" => $this->input->post('email'),
                        "customerCityName" => $this->input->post('cityid'),
                        "customerDistrictName" => $this->input->post('districtid'),
                        "customerAddress" => $this->input->post('address'),
                        "productList" => $productList,
                    );

                    $response = sendRequestNhanhVN('/api/order/add', $data_arr);
            ...
            if (isset($_product_) && !empty($_product_)) {
                ...
                $this->mailbie->sent(array(
                    ...
                    'message' => mail_html(array(
                        ...
                        'total_price_1' => ($_payment['total_price'] + $total_ship - $price_gift - $_payment['coint']),
